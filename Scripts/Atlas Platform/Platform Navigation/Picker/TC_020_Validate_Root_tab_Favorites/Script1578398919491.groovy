...
List<WebElement> listofoptions = WebUiCommonHelper.findWebElements(findTestObject('Swicther/root_tab_all_values'), 5)

List<WebElement> listoffavicons = WebUiCommonHelper.findWebElements(findTestObject('Swicther/root_tab_fav_icon'), 5)
...
println(listofoptioninstring)

String verify_accountName = 'Root'

for (int h = 0; h < listofoptions.size(); h++) {
    if ((listofoptions.get(h).getText().split('Last')[0]).trim().equalsIgnoreCase(verify_accountName)) {
...
for (int j = 0; j < listoffavvalues.size(); j++) {
    if ((listoffavvalues.get(j).getText().split('Last')[0]).trim().equalsIgnoreCase(verify_accountName)) {
        isPresent = true
    }
}
