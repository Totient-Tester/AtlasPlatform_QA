...
List<WebElement> listofAccNames = WebUiCommonHelper.findWebElements(findTestObject('Swicther/organization_tab_all_values'), 
    10)

List<WebElement> textValuesForOrgName = new ArrayList<String>()

for (int hjk = 0; hjk < listofAccNames.size(); hjk++) {
    textValuesForOrgName.add((listofAccNames.get(hjk).getText().split('Last')[0]).trim())
}
...
boolean isPresent = textValuesForOrgName.removeAll(rootNames)

WebUI.verifyEqual(isPresent, false)

WebUI.click(findTestObject('Swicther/accounts_tab_swicther'))

List<WebElement> listofOrgNames = WebUiCommonHelper.findWebElements(findTestObject('Swicther/accounts_tab_all_values'), 
    10)
...
isPresent = textValuesForOrgName.removeAll(orgNames)
