...
for (j = 1; j < 3; j++) {
    WebUI.click(findTestObject('ROLES/Roles_page/sorting/Created_header'))

    List<WebElement> createdDate

    List<WebElement> AllCreatedDate = new ArrayList<String>()

    for (int i = 1; i <= noOfPages; i++) {
        createdDate = WebUiCommonHelper.findWebElements(findTestObject('ROLES/Roles_page/sorting/created'), 2)

        for (WebElement a : createdDate) {
            AllCreatedDate.add(a.getText())
        }
    ...
    if (j == 1) {
        boolean isSorted = CustomKeywords.'web.CustomKeyword.checkListDescending'(AllCreatedDate)

        println(AllCreatedDate.size())

        println(AllCreatedDate)
    ...
    if (j == 2) {
        boolean isSorted = CustomKeywords.'web.CustomKeyword.checkListAscendingOrder'(AllCreatedDate)
