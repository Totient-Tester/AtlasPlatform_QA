...
for (j = 1; j < 3; j++) {
    WebUI.click(findTestObject('ROLES/Roles_page/sorting/RoleName_header'))

    List<WebElement> RoleNames

    List<WebElement> AllRoleNames = new ArrayList<String>()

    for (int i = 1; i <= noOfPages; i++) {
        RoleNames = WebUiCommonHelper.findWebElements(findTestObject('ROLES/Roles_page/Role Names'), 2)

        for (WebElement a : RoleNames) {
            AllRoleNames.add(a.getText().toLowerCase())
    ...
    if (j == 1) {
        boolean isSorted = CustomKeywords.'web.CustomKeyword.checkListDescending'(AllRoleNames)

        println(AllRoleNames.size())
    ...
    if (j == 2) {
        boolean isSorted = CustomKeywords.'web.CustomKeyword.checkListAscendingOrder'(AllRoleNames)
