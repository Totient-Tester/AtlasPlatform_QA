...
for (j = 1; j < 3; j++) {
    ...
    if (j == 1) {
        boolean isSorted = CustomKeywords.'web.CustomKeyword.checkListDescending'(AllStatus)

        println(AllStatus.size())
    ...
    if (j == 2) {
        boolean isSorted = CustomKeywords.'web.CustomKeyword.checkListAscendingOrder'(AllStatus)
