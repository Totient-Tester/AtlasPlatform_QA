...
for (j = 1; j < 3; j++) {
    WebUI.click(findTestObject('ROLES/Roles_page/sorting/Modified_header'))

    List<WebElement> modifiedDate

    List<WebElement> AllModifiedDate = new ArrayList<String>()

    for (int i = 1; i <= noOfPages; i++) {
        modifiedDate = WebUiCommonHelper.findWebElements(findTestObject('ROLES/Roles_page/sorting/modified'), 2)

        for (WebElement a : modifiedDate) {
            AllModifiedDate.add(a.getText())
        }
    ...
    if (j == 1) {
        boolean isSorted = CustomKeywords.'web.CustomKeyword.checkListDescending'(AllModifiedDate)

        println(AllModifiedDate.size())

        println(AllModifiedDate)
    ...
    if (j == 2) {
        boolean isSorted = CustomKeywords.'web.CustomKeyword.checkListAscendingOrder'(AllModifiedDate)
